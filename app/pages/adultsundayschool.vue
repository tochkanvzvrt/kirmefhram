  ...
  <div class="w-full">
    <section class="bg-gradient-to-br from-primary to-primary/80 py-20 text-white">
      <div class="mx-auto px-4 lg:px-8 text-center container">
        ...
        <h1 class="mb-4 font-serif text-5xl md:text-6xl">Воскресная школа для взрослых</h1>
    ...
    <section class="bg-white py-16">
      <div class="mx-auto px-4 lg:px-8 container">
        ...
        <div v-else-if="adultsundaySchoolItems.length === 0" class="py-16 text-center">
        ...
        <div v-else class="gap-8 grid grid-cols-1 md:grid-cols-2 mx-auto mb-8 max-w-5xl">
          <Card v-for="item in adultsundaySchoolItems" :key="`${item.type}-${item.id}`"
...
const fetchSundaySchool = async () => {
  try {
    const res = await fetch('http://localhost/church/wp-json/wp/v2/adultsundayschool')
...
const adultsundaySchoolItems = computed(() => {
